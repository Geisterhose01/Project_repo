...
st.set_page_config(page_title="Tinder for Recipes", page_icon="🍴") #Konfiguration der Standardeinstellungen der Seite
...
st.title("Tinder for Recipes") #Titel der Seite
...
def main():
    ingredients = st.text_input("Insert your ingredients with a comma ','. (e. g. chicken, rice, broccoli): ") #Eingabefeld für die gewünschten Zutaten
    diet = st.selectbox("Preferences", ["None", "Vegetarian", "Vegan", "Gluten Free", "Ketogenic"]) #Auswahl der Diätpräferenzen

    if st.button("Search Recipes"): #Button für die Rezeptsuche
        if ingredients: #Prüfen, ob eine Zutat hinzugefügt wurde
            ...
            if len(results) == 0: #wenn die Länge von results == 0 ist, wurden keine Rezepte gefunden
                st.write("No recipes found. Try again.")
            else:
            ...
            st.write("Add at least one ingredient.")
